...
/**
 * Creates a custom shortcode [mofire_upcoming_events] to display the next 3 upcoming events in a card grid.
 * NOW INCLUDES THE FEATURED IMAGE.
 */
function mofire_display_upcoming_events_shortcode() {
    // Use an output buffer to capture the HTML
    ob_start();

    // Arguments for the events query
    $args = array(
        'post_type'      => 'tribe_events', // The Events Calendar post type
        'posts_per_page' => 3,              // We only want 3 events
        'eventDisplay'   => 'upcoming'       // A special parameter from the plugin to only get future events
    );

    $upcoming_events = new WP_Query($args);

    if ($upcoming_events->have_posts()) :
        echo '<div class="mofire-upcoming-events-list">';
            ...
            <article class="mofire-event-item">
                ...
                if (has_post_thumbnail()) : ?>
                    <a href="<?php the_permalink(); ?>" class="mofire-event-image-link">
                        <?php the_post_thumbnail('medium_large'); // Use a good size like 'medium_large' or 'medium' for cards ?>
                    </a>
                <?php endif; ?>
...
add_shortcode('mofire_upcoming_events', 'mofire_display_upcoming_events_shortcode');